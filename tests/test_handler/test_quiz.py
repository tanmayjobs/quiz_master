...
class TestQuizHandler:
    def test_add_quiz_negative(self, mock_user):
        with raises(ValueError):
            QuizHandler(mock_user).add_quiz()

    def test_add_quiz_positive(self, mock_user, mock_quiz, mock_db_context):
    ...
    def test_get_random_quiz_negative(self, mock_db_context):
        with patch("src.handler.quiz.DBContext", mock_db_context):
            mock_db_context.read.return_value = None
            expected_value = None
            assert expected_value == QuizHandler.get_random_quiz()

    def test_get_random_quiz_positive(self, mock_db_context):
        with patch("src.handler.quiz.DBContext", mock_db_context):
            quiz_data = (
                1,
                "Quiz Name",
    ...
    def test_remove_quiz_negative(self, mock_user, mock_db_context):
        with raises(ValueError):
            with patch("src.handler.quiz.DBContext", mock_db_context):
                mock_db_context.write.return_value = False
                QuizHandler(mock_user).remove_quiz()

    def test_remove_quiz_positive(self, mock_user, mock_quiz, mock_db_context):
        with patch("src.handler.quiz.DBContext", mock_db_context):
