...
class AuthServices:
    ...
    def sign_in(self, username, password):
        ...
        if not user_data or not check_password(password, user_data["hash_password"]):
            raise InvalidCredentials(401, "Unauthorized", "username and password do not match!")
    ...
    def sign_up(self, username, password):
        ...
        try:
            with self.database_access as dao:
                dao.write(SQLQueries.ADD_USER, (uuid.uuid4(), username, password_hash, UserRole.PLAYER.name),)
        except IntegrityError:
            raise AlreadyExists(409, "Conflict", "username already exists!")
